...
impl DBFile {
    ...
    fn all_senses<'db, 'file>(&'file self, database: &'db Database, vector: &mut Vec<(SenseId, Sense<'db>)>) {
        let mut buf = String::new();
        self.index.read_to_string(&mut buf).unwrap();

        for line in buf.lines().skip_while(|l| l.starts_with("  "))
        //skip license lines
        {
            let senses = Self::parse_senses2(&self, database, line);
            vector.extend(senses);
        }
    }
    ...
    fn read_sense<'db>(&self, database: &'db Database, offset: u64) -> Sense<'db> {
        ...
        if sections[2] == b"v" {
            let frame_count = match usize::from_str_radix(from_utf8(sections[index]).unwrap(), 10)
                .map_err(|err| {
                    format!(
                        "Error '{err}' parsing frame count '{}' in line '{}' ",
                        unsafe {
                            String::from_utf8_unchecked(sections[index].iter().copied().collect())
                        },
                        unsafe { String::from_utf8_unchecked(line.clone()) }
                    )
                }) {
                Ok(fc) => fc,
                Err(err) => {
    ...
    fn senses<'db>(&self, database: &'db Database, word: &[u8]) -> Option<Vec<Sense<'db>>> {
        ...
        let v: Vec<_> = self.parse_senses(database, &line).collect();

        Some(v)
    }

    fn parse_senses<'db, 's, 'l>(
        &'s self,
        database: &'db Database,
        line: &'l str,
    ) -> impl ExactSizeIterator<Item = Sense<'db>> + use<'db, 'l, 's> {
        let sections: Vec<&'l str> = line.split(' ').collect();
        ...
        let ptr_symbols_cnt = sections[index];
        let ptr_symbols_cnt = match sections[index].parse::<usize>() {
            Ok(i) => i,
            Err(err) => {
                panic!("Error '{err}' parsing '{ptr_symbols_cnt}' as usize in line '{line}'")
            }
        };
        ...
        let iter = (0..synset_cnt).map(move |synset| {
            let synset_offset = u64::from_str_radix(sections[index + synset as usize], 10).unwrap();
            self.read_sense(database, synset_offset)
        });
        iter
    }

    fn parse_senses2<'db, 's, 'l>(
        &'s self,
        database: &'db Database,
        line: &'l str,
    ) -> impl ExactSizeIterator<Item = (SenseId, Sense<'db>)> + use<'db, 'l, 's> {
        let sections: Vec<&'l str> = line.split(' ').collect();
        ...
        let iter = (0..synset_cnt).map(move |synset| {
            let synset_offset = u64::from_str_radix(sections[index + synset as usize], 10).unwrap();
            let sense = self.read_sense(database, synset_offset);
            let sense_id = SenseId {
...
impl WordId {
    pub fn new(lex_id: u32, part_of_speech: PartOfSpeech) -> Self {
        Self { lex_id, part_of_speech }
    }
}
...
impl Database {
        ...
        'files: for e in std::fs::read_dir(path)? {
                ...
                == std::ffi::OsStr::new("index")
            {
                ...
                let part_of_speech = if ex == "noun" {
                    ...
                    // 	std::io::ErrorKind::InvalidData,
                    // 	format!("file '{}' has invalid part of speech", path_buf.as_os_str().to_string_lossy())
                    // ));
                };
    ...
    pub fn senses<'db>(&'db self, word: &str) -> Vec<Sense<'db>> {
        let mut all = vec![];
    ...
    pub fn all_senses<'db>(&'db self) -> Vec<(SenseId, Sense<'db>)> {
        let mut all = vec![];
        for db_file in &self.db_files {
            db_file.all_senses(&self, &mut all);
        }
        all
    }
}
...
mod test {
    #[test]
    fn test_1() {
        let wn = crate::wordnet::Database::open(&::std::path::Path::new(
        ...
        assert_eq!(
            ...
            wn.senses("bank")[2]
                .pointers
                .iter()
                .filter(|&x| x.relationship == crate::wordnet::Relationship::Hypernym)
                .count()
        );
